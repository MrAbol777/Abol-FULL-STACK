...
loginForm.addEventListener('submit', (e) => {
    ...
    // Check credentials - STRICT validation
    const usernameMatch = username.toLowerCase().trim() === AUTH_CONFIG.username.toLowerCase().trim();
    const passwordMatch = password === AUTH_CONFIG.password;
    ...
    if (usernameMatch && passwordMatch) {
        ...
        const sessionData = {
            authenticated: true,
            username: AUTH_CONFIG.username, // Store username for verification
            expires: expires,
            timestamp: new Date().getTime(),
            ip: 'local' // Can be enhanced with actual IP
        };
        localStorage.setItem(AUTH_CONFIG.sessionKey, JSON.stringify(sessionData));
        ...
        // Redirect to admin panel with replace (prevents back button)
        setTimeout(() => {
